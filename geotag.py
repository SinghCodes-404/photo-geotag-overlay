...
Nominatim), fetches an OSM map thumbnail, and composites a bottom banner:
    ...
    [ map ]   <bold place title>
              ...
              Saturday, 04/07/2026 11:58 AM GMT +05:30

The date/time line comes from the photo's EXIF DateTimeOriginal (set the correct
times first with set_datetime.py). The weekday is derived from the date, so it
can never mismatch. Set SHOW_DATETIME=False to omit the line.

Usage:
    python3 geotag.py                     # every *.jpg in this folder (flat)
    python3 geotag.py IMG20260625174721.jpg   # specific file(s)
    python3 geotag.py Social_Internship   # walk a folder tree, mirror the output

Output goes to ./geotagged/. For a folder tree the structure is mirrored under
geotagged/ (e.g. geotagged/Social_Internship/2026-06-24/Morning_0900/…).
...
SHOW_DATETIME = True            # append the day/date/time line to the banner
TZ_LABEL = "GMT +05:30"         # timezone suffix shown after the time (IST)
...
# --------------------------------------------------------------------------
def _to_float(x):
    """Convert an EXIF rational to float, or None if it's invalid (e.g. a
    0/0 placeholder that some phones write when there is no real GPS fix)."""
    try:
        f = float(x)
...
def get_gps(path):
    """Return (lat, lon) decimal degrees, or None if there is no valid fix."""
    ...
    def dms(vals):
        parts = [_to_float(x) for x in vals]
    ...
    if gps.get("GPSLongitudeRef") == "W":
        lon = -lon
    # reject out-of-range or the (0, 0) "null island" placeholder
...
def get_datetime(path):
    """Return a datetime from EXIF DateTimeOriginal (fallback DateTime), or None."""
    exif = Image.open(path)._getexif()
    if not exif:
        return None
    by_name = {TAGS.get(k, k): v for k, v in exif.items()}
    raw = by_name.get("DateTimeOriginal") or by_name.get("DateTime")
    if not raw:
        return None
    try:
...
def render(path, out_path):
    ...
    if SHOW_DATETIME:
        ...
        if dt:
            stamp = f"{dt:%A, %d/%m/%Y %I:%M %p} {TZ_LABEL}"
            lines.append((stamp, body_f))
...
def _is_jpg(f):
    return f.lower().endswith((".jpg", ".jpeg"))


def collect(args):
    """Yield (input_path, output_path) pairs.

    - a file arg          -> flat into OUT_DIR/
    - a directory arg     -> walked recursively, structure mirrored under OUT_DIR
    - no args             -> flat *.jpg in the current folder (excluding OUT_DIR)
    """
    if not args:
        for f in sorted(os.listdir(".")):
            if _is_jpg(f) and "geotagged" not in f:
                yield f, os.path.join(OUT_DIR, f)
        return
    for a in args:
        if os.path.isdir(a):
            root_parent = os.path.dirname(os.path.normpath(a))
            for dirpath, _, files in os.walk(a):
                ...
                for f in sorted(files):
                    if _is_jpg(f):
                        ...
                        rel = os.path.relpath(src, root_parent) if root_parent else src
                        yield src, os.path.join(OUT_DIR, rel)
        elif _is_jpg(a):
            yield a, os.path.join(OUT_DIR, os.path.basename(a))


def main():
    pairs = list(collect(sys.argv[1:]))
    ok = 0
    for src, dst in pairs:
        ...
        try:
            ...
            print(f"  ! error: {e}")
    print(f"\nDone: {ok}/{len(pairs)} images written to {OUT_DIR}/")
